...
d$X  <- x3$m
d$s  <- x3$s
constants = list(N=nrow(x3),a=5000,b=3001)
...
out.3  <- parLapply(cl=cl,X=seeds,fun=run,d=d,constants=constants,niter=niter,nburnin=nburnin,thin=thin)
stopCluster(cl)

# Diagnostic and Posterior Processing
post.sample.3  <- coda::mcmc.list(out.3)
diag.3  <- coda::gelman.diag(post.sample.3)
print('Diagnostics:\n')
print(diag.3$psrf)
save(post.sample.3,diag.3,file=here('simulation','sim_analyses_3.RData'))
