...
d$X  <- x1$m
d$s  <- x1$s
constants = list(N=nrow(x1),a=5000,b=3001)
...
post.sample.1  <- coda::mcmc.list(out.1)
diag.1  <- coda::gelman.diag(post.sample.1)
print('Diagnostics:\n')
print(diag.1$psrf)
save(post.sample.1,diag.1,file=here('simulation','sim_analyses_1.RData'))
